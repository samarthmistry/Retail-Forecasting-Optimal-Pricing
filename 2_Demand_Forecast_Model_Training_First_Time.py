...
if not model_exist:
    ...
    df_model_performance = pd.DataFrame([{'model_time': model_time, 'RMSE': np.sqrt(metrics.mean_squared_error(y, predictions)), 'R2': metrics.r2_score(y, predictions)}])
    ...
    model_name = pd.DataFrame([{'model_name': dirfilename}])
